...
public:
   ...
   **/
  void
  visualizePlannerData(const LocalPlanner &planner,
   ...
   **/
  void
  publishTree(const std::vector<TreeNode> &tree,
   ...
   * @params[in]  goal, the loaction of the goal used in the planner
   *calculations
   **/
   ...
   **/
  void
  publishDataImages(const std::vector<uint8_t> &histogram_image_data,
   ...
   * @params[in]  adapted_goto_position, desired direction adapted to the
   *desired speed
   * @params[in]  smoothed_goto_position, smoothed waypoint which is given to
   *the controller
   **/
   ...
   * @params[in]  last_wp, location of the smoothed waypoint at the last
   *timestep
   ...
   *              current timestep
   **/
  void publishPaths(const Eigen::Vector3f &last_position,
                    const Eigen::Vector3f &newest_position,
                    const Eigen::Vector3f &last_wp,
                    const Eigen::Vector3f &newest_wp,
   ...
   * @params[in]  closest_pt, vehicle position projection on the line previous
   *to current goal
  ...
  void publishRangeScan(const sensor_msgs::LaserScan &scan,
                        const Eigen::Vector3f &newest_position) const;
